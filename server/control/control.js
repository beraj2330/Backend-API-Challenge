...
const route2 = (req, res) => {
  ...
  // Throws error if the sortBy value are not valid.
  ...
  // Throws error if the direction value are not valid.
  ...
  if (tags.indexOf(',') !== - 1) {
    ...
    axios.all([...getPaths])
      ...
      .then(axios.spread((tag1, tag2, tag3, tag4, tag5, tag6, tag7, tag8, tag9) => {
        ...
        // Most efficient way to get rid of duplicates is to make a hash table using the post id as the key in the hash table, it will ensure that duplicates are removed.
        let post = {};
        let posts = [];
        ...
        // Removed duplicates and sorted data as the user desires and can return the data.
        res.status(200).send(posts);
      }))
  } 
  else {

    // User searches with only one tag
...
module.exports = {route1,route2,error}