...
const axios = require('axios');
  ...
  describe('Route 2', function() {
    it('Should return the proper status code for route 2 for the correct route', function(done) {
      request('http://localhost:3000/api/posts/tech', function(error, response, body) {
        expect(response.statusCode).to.equal(200);
        done();
      });
    });
    it('Should return the correct status code for route 2 where route is incorrect', function(done) {
      request('http://localhost:3000/api/post/tech', function(error, response, body) {
          expect(response.statusCode).to.equal(404);
          done();
      });
    });
    ...
    it('Should return the correct status code for route 2 when the user uses all three parameters', function(done) {
      request('http://localhost:3000/api/posts/science,tech/popularity/desc', function(error, response, body) {
          expect(response.statusCode).to.equal(200);
          done();
      });
    });
    it('Should return the correct status code for route 2 when the user uses all three parameters', function(done) {
      request('http://localhost:3000/api/posts/health,tech/likes/asc', function(error, response, body) {
          expect(response.statusCode).to.equal(200);
          done();
      });
    });
    it('Should pass the test if all posts are unique by checking unique ids', function(done) {
      axios.get('http://localhost:3000/api/posts/tech,history')
      .then(res => {
        let post = res.data;
        let postID = [];
        let postObj = {};
    ...
    it('Should pass the test if all posts are unique by checking unique ids using all route parameters', function(done) {
      axios.get('http://localhost:3000/api/posts/tech,history/likes/asc')
      .then(res => {
        let post = res.data;
        let postID = [];
        let postObj = {};
    ...
    it('Should pass the test for correctly sorted values', function(done) {
      axios.get('http://localhost:3000/api/posts/tech,health/likes/desc')
      .then(res => {
        let post = res.data;
        let postLikes = [];
        let test = true;
    ...
    it('Should pass the test for correctly sorted values given the default parameters, as if the user did not use them', function(done) {
      axios.get('http://localhost:3000/api/posts/tech,health')
      .then(res => {
        let post = res.data;
        let postID = [];
        let test = true;
